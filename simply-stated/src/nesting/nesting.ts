...
> = {
  ...
  >
    ? TransitionsToUnexpectedState<
      ...
      > extends true
      ? UsageGuardError<`Forwarded event '${EventName}' transitions to an unexpected inner state (${PresentStateNames<
    ...
    : UsageGuardError<`Forwarded event '${EventName}' is not handled by any inner state (${PresentStateNames<ExpectedInnerState>})`> &
        ((...args: any[]) => never);
};
