...
describe('toCollectionStore', () => {
  ...
  describe('collectionPath', () => {
    const setup = () => {
      const machine = makeFetchMachine();
      const store = createStore(
        toCollectionStore(machine, { collectionPath: 'entities.fetches' }),
      );
      return { machine, store };
    };
  ...
  describe('adjustActions', () => {
    ...
    it('renames the lifecycle methods while keeping them bound', () => {
      const { machine, store } = setup();
    ...
    it('the adapter still places the collection at collectionPath', () => {
      const { store } = setup();
      expect(store.getState().jobs).toEqual({});
    });

    it('merges the collection into a branch the actions also use', () => {
      const machine = makeJobMachine();
      const store = createStore(
        toCollectionStore(machine, {
      ...
      store
        .getState()
        .machines.jobActions.addEntity(
          machine.state.Queued({ id: 'j1', n: 0 }),
        );
      expect(store.getState().machines.jobs).toEqual({
        j1: { name: 'Queued', data: { id: 'j1', n: 0 } },
      });
    });
  });
});
