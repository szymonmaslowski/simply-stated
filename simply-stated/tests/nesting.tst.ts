...
test('a forwarded event no target state handles is a dead-forward error', () => {
  combineStates(
    ...
    Loading: {
      // @ts-expect-error UsageGuardError<"Forwarded event 'refetch' is not handled by any inner state ('Fetching', 'Failure')">
...
test('spreading a map with a dead event is rejected at the state', () => {
  combineStates(
    ...
    // @ts-expect-error UsageGuardError<"Forwarded event 'fetch' is not handled by any inner state ('Fetching', 'Failure')">
    Loading: {
...
test('an escaping forwarded event errors at the event slot', () => {
  combineStates(
    ...
    Outer: {
      // @ts-expect-error UsageGuardError<"Forwarded event 'toC' transitions to an unexpected inner state ('C')">
